...
public class huffman_encode {
    huffman_encode(String input_file_name, String output_file_name)
    {
        this.input_file_name = input_file_name;
        this.out_file_name = output_file_name;
    ...
    public void read_file_to_coding() {
        ...
        try {
            ...
            while ((c = br.read()) != -1) //read() читает один байт из потока
            {
                //count_character++;
    ...
    public void coding(Node root, String str, HashMap<Integer, String> map_code)
    {
        if (root == null)
            return;
    ...
    {
        try(FileOutputStream fos = new FileOutputStream(this.out_file_name);
            OutputStreamWriter osw = new OutputStreamWriter(fos, StandardCharsets.UTF_8);
            BufferedWriter writer = new BufferedWriter(osw))
        {
            Path path = Paths.get(input_file_name);
            int size_file = (int) Files.size(path);
            String string_count_char = Integer.toBinaryString(size_file); //запишем размер исходного файла
            ...
            for (HashMap.Entry<Integer, String> entry : map_coding.entrySet()) {

                int ch = entry.getKey();
                int charIntcode = Integer.parseInt(Integer.toBinaryString(ch), 2);
            ...
            for (int i = 0; i < original_line.length(); i++)
            {
                ...
                String code_line = map_coding.get((int)ch);
    ...
    private void write_chunk(BufferedWriter writer, String string, boolean drop_len) throws IOException {
        int chunkSize = 8;
    ...
    private HashMap<Integer, String> map_coding; //словарь для сохранения кодировок букв
